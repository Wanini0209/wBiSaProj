...
- 將所有受版本控制的專案文件視為人機溝通與系統執行的單一事實來源——在各文件所定義的主題範圍內，文件即為該主題的正式依據
...
> - **`<impl_file>`**：私有實作檔案的相對路徑與檔名（相對於 `<fu_path>`），必須位於 Feature 級私有目錄內（例如：`_json_io/_json.py`）。詳見《架構篇》§4.3「Feature 級私有實作隔離」。
>
