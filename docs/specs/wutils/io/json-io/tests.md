...
|------|---------|--------|---------|
| test_json_io.py | json_io | overall | 驗證 json_dump / json_load 的完整功能，包含 UTF-8 編碼、路徑支援、參數透傳、異常透傳與讀寫循環 |

> 本 FU 目前規模適中，以單一測試檔涵蓋所有案例。若未來案例顯著增長，應依 `test_json_io__<aspect>.py` 模式拆分。
...
- **Target File**:
  - `tests/wutils/io/json-io/test_json_io.py`
  - 若未來拆分：`tests/wutils/io/json-io/test_<subject>__<aspect>.py`
...
```bash
# 執行本 FU 的單元測試
pytest tests/wutils/io/json-io/ -v -m unit

# 執行並產生覆蓋率報告
pytest tests/wutils/io/json-io/ -v --cov=wutils/io --cov-report=term-missing
```
