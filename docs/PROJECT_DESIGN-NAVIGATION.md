...
本文件是 wBiSaProj 專案 root-level 核心設計文件體系的一部分，定位為**治理型核心篇章**。本文件負責定義專案的文件導航體系與多層級入口治理規則，包括 Overview 文件的三層導航架構、目錄結構映射規則、各層級 Overview 的內容標準，以及支撐團隊開發與 LLM 協作的可導航性機制。
