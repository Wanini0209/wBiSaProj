...
本專案的文件體系中，不同類型的文件具有不同層級的權威性。為避免讀者在遇到衝突時無法判斷以誰為準，各類文件在描述自身或其他文件的權威性時，應遵循以下層級模型：

| 層級 | 適用對象 | 權威範圍 | 用語建議 |
|:-----|:---------|:---------|:---------|
| **Project-wide** | root-level 核心設計文件（`PROJECT_DESIGN*.md`） | 整個專案的最高治理依據 | 可使用「專案核心設計依據」 |
| **Topic-wide** | standards 規範文件（如 `version-control.md`、`testing.md`） | 該主題範圍內的正式規範來源 | 可使用「在 X 主題範圍內的正式規範來源」 |
| **System / Domain-local** | 系統或領域級 architecture overview | 該系統或領域內部的正式依據 | 應明確標示所屬系統或領域範圍 |
| **Feature / FU-local** | use-case、spec、test plan | 該 Feature 或 FU 的局部正式依據 | 應明確標示所屬 Feature 或 FU 範圍 |
| **Guide / Explanatory** | 實作指引、寫作指南、導讀文件 | 解釋與示範，不自行定義正式規範 | 不應宣稱自身為最高權威；如需引用正式依據，應導回上游文件 |

使用「SSOT」、「Ground Truth」、「憲法」、「唯一權威來源」等強權威詞彙時，必須清楚交代其適用範圍（例如「在該 FU 的技術規格範圍內」），避免被誤讀為 project-wide 絕對最高權威。Guide 類文件原則上不應使用此類詞彙描述自身的權威性。
...
為確保專案結構的穩定性與可擴展性，我們制定了強制性規範，要求所有實作細節必須私有化，並將 `__init__.py` 作為唯一的公開入口。此外，在 FU Container 內部，所有私有實作檔案必須按 Feature 劃分至專屬的私有目錄中（`_<feature_name>/`），確保從公開介面到私有實作的歸屬鏈完整性（詳見架構篇 §4.3）。此設計旨在從架構層面強制實現介面與實作的分離。為精準反映技術概念本身的層次結構，這些容器允許被嵌套，這與業務領域中 Domain/Sub-Domain 的組織方式是相互對應的。
...
1. **依賴管理正確性**: 跨公開邊界的依賴使用正式絕對路徑，邊界內部依賴使用相對 import，封裝邊界未被穿透。
...
- **靜態規範 (Static Norms)**：專案預先定義的、跨任務通用的規則與標準，在 LLM 協作情境中扮演類似「憲法」的角色——即不因具體任務而改變的穩定規則基底。例如：命名規範、架構原則。
